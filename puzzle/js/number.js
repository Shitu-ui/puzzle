...
function userGuessed() {
	...
	if (userGuessed.length == 0 || ! guessInRange(userGuessed)) {
		...
		if (userGuessed == secretNumber) {
			...
			storeMax(numberOfGuesses);
			window.location.replace('riddle.html');
...
function storeMax(numberOfGuesses) {
	alert("Hello");
	...
	const data = "numberOfGuesses=" + encodeURIComponent(numberOfGuesses);
	xhr.send(data);
}
